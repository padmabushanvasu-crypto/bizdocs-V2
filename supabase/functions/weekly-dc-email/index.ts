...
//
// Sister to weekly-po-email. Triggered daily; for each company we read
// dc_email_day from company_settings and only send if today matches.
// Returnable DCs only (dc_type = 'returnable').
//
// Output: 4-sheet styled .xlsx workbook covering DCs raised in the last
// 7 days, open returnable DCs with return-due alerts, overdue returns,
// and partially returned DCs.
...
serve(async (_req) => {
  try {
    ...
    for (const company of companies as any[]) {
      ...
      // ── Fetch DCs (last 7 days raised + all open) — returnable only ────
      const { data: lastWeekDCs } = await supabase
        ...
        .gte("dc_date", sevenDaysAgo)
        .lte("dc_date", todayStr)
      ...
      // ── Sheet 1 — DCs Raised (Last 7 Days) ─────────────────────────────
      ...
      for (const dc of (lastWeekDCs ?? []) as any[]) {
      ...
      // ── Summary numbers ────────────────────────────────────────────────
      const dcRaisedCount = (lastWeekDCs ?? []).length;
      const openCount = (openDCs ?? []).length;
      const overdueCount = overdueDCs.length;
      const partialCount = partialDCs.length;

      const fileName = `DC_Weekly_${todayStr}.xlsx`;
      const emailBody = `Weekly DC Summary — ${company.company_name ?? ""}

Returnable DCs raised in the last 7 days: ${dcRaisedCount}
Open DCs awaiting return: ${openCount}
Overdue returns: ${overdueCount}
Partially returned: ${partialCount}
