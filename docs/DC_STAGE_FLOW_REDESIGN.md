...
| **4 — Loops** | Both | **In progress (5 Sep 2026).** ✅ Scrap register unification: `rpc_dispose_rejected` and `rpc_return_or_scrap_wip` both now write to `scrap_register` (previously stock_ledger only). Along the way, found and fixed a real bug unrelated to this session's own code: `scrap_register.assembly_order_id`'s FK pointed at `assembly_orders`, a table with 0 rows that was superseded by `assembly_work_orders` — meaning no AWO-linked scrap row could *ever* have been written, regardless of any application code. Repointed the FK; `fat_certificates` and `serial_numbers` have the identical stale FK, both also at 0 rows, flagged but deliberately untouched (out of scope, unaudited). `grn_scrap_items` deliberately left separate — judged to be a different concept (bulk material waste by type, not item-level) pending confirmation. ✅ Rework cycle visibility: `v_job_card_stage_ledger_totals.rework_cycle_count` added — derived by counting `rework_in` ledger rows per stage, no new stored column, consistent with the derived-not-stored principle. Intended as a signal (repeated failure → consider scrap instead of rework again), not a hard rule. **Not started:** damage disposition (A2) — discovered `awo_line_items.damage_qty/damage_reason/disposition/concession_*` columns already exist with a `disposition` CHECK (`scrap`/`use_as_is`/`return_to_vendor`), 23,706 rows, 0 using any of these columns — same "designed, never wired" pattern as `scrap_register` was. Weldment pool accumulation (`pool` batch type) and sub-store (blocked on the client's qty-per mapping sheet). | Weldment, multi-batch, sub-store |
