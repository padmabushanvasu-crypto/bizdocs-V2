...
**Date:** 5 Sep 2026 (living document — updated in place as phases complete)  
...
**Status:** Phases 0, 1, 2 complete and live. Phase 3a (forward-path UI) built and pushed, pending one live click-through and PR merge. Phase 3b UI partially wired (reverse-return, dispose-rejected dialogs live); Phase 4 in progress; 6 Sep 2026 audit fixes and confirm-at-receipt deployed — see §11a. **Canonical location: `docs/DC_STAGE_FLOW_REDESIGN.md` in the repo** — this file was previously only ever kept outside the repo, which caused Claude Code to correctly stop and report it missing on 5 Sep 2026; it has since been committed and should be read from there going forward, not regenerated.
...
| **3a — UI, forward path** | Claude Code | **✅ COMPLETE (5 Sep 2026), pending merge + one live click-through.** Job card creation (new standalone "Open Job Card" flow, legacy DC-triggered dialog left untouched) → `rpc_open_job_card`; internal-stage confirm button on the job-card detail page → `rpc_confirm_internal_step`; DC issuance with the four-rule ask/auto-fill picker (`JobCardLinePicker`) → `rpc_issue_dc`; GRN store-confirm → `rpc_confirm_grn_store`. Both `issueDeliveryChallan` and `storeConfirmGRNItems` branch on whether the line resolves to a job-card-linked, non-legacy line — if so, skip the old direct stock-posting and call the new RPC instead; legacy lines are untouched. Branch `claude/dc-stage-flow-forward-path`, 5 commits (one concern each), `tsc`/`build` clean throughout. Caught and self-corrected mid-task: a commit had landed directly on `main` — reset before push, confirmed `main` untouched. Verified end-to-end with a `BEGIN…ROLLBACK` against a real live item (230226): stock debited once at open, unchanged through an intermediate non-final GRN return, credited back exactly at the true final stage — confirmed independently afterward with zero residual trace. One real gap found and closed same-day: `rpc_confirm_grn_store` originally errored if a GRN's lines were confirmed across more than one partial call; changed to skip already-confirmed lines silently (matches `rpc_issue_dc`'s existing idempotent pattern) — DB-only fix, no frontend change needed. **Still needed before wide rollout:** one real click-through in the deployed UI (the SQL proof cannot catch a React wiring/query-key typo) and the PR merge itself. | Phase 3b |
| **3b — UI, backward path** | Claude Code | Not started. Editing/cancelling an issued DC line, reversing a GRN return, disposing rejected material (rework/scrap), cancelling or closing a job card short — RPCs exist and are tested (`rpc_update_dc_line_qty`, `rpc_cancel_dc`, `rpc_reverse_grn_return`, `rpc_undo_internal_step`, `rpc_dispose_rejected`, `rpc_cancel_job_card`, `rpc_close_job_card_short`), UI wiring is a separate, similarly-sized handoff. | Full parity with old system |
| **4 — Loops** | Both | **In progress (5 Sep 2026).** ✅ Scrap register unification (A3): `rpc_dispose_rejected` and `rpc_return_or_scrap_wip` both now write to `scrap_register` (previously stock_ledger only). Found and fixed a real bug unrelated to this session's own code along the way: `scrap_register.assembly_order_id`'s FK pointed at `assembly_orders`, a table with 0 rows superseded by `assembly_work_orders` — no AWO-linked scrap row could *ever* have been written. Repointed the FK; `fat_certificates`/`serial_numbers` have the identical stale FK, both also 0 rows, flagged but untouched (out of scope, unaudited). **`grn_scrap_items` (10 rows) confirmed by product owner to be the same concept as `scrap_register`, not a separate one as first assumed** — unified via an `AFTER INSERT` trigger (`trg_grn_scrap_items_feed_register`) that mirrors every future `grn_scrap_items` row into `scrap_register` automatically, with a one-time backfill of the existing 10. The GRN-side write path is completely unchanged — the trigger does the unification invisibly. `ScrapRegister.tsx` already reads `scrap_register` broadly, so these rows are visible with zero UI change. ✅ Rework cycle visibility: `v_job_card_stage_ledger_totals.rework_cycle_count` — derived from `rework_in` ledger rows, no new column, surfaced as an informational warning (≥2 cycles) in the dispose-rejected dialog — shipped to production. ✅ Damage disposition (A2), fully done including UI: discovered `awo_line_items.damage_qty/damage_reason/disposition/concession_*` already existed with a working CHECK constraint, 23,706 rows, 0 ever used — same "designed, never wired" pattern as scrap_register. Three decisions grounded in standard MRB (Material Review Board) practice rather than company data (none existed to learn from): damage is reported at *return* from the shop floor, not at issuance; `use_as_is` requires a `qc_team`/`admin` role via the real `profiles.role` column; `return_to_vendor` records the decision and removes the stock but does **not** attempt to generate a debit note — no such document type exists in BizDocs, named as a future integration point. New column `awo_line_items.damage_resolved_qty` (cumulative, symmetric with issued/returned/scrapped/consumed; pending = damage_qty − damage_resolved_qty). Two new RPCs, `rpc_report_damage` and `rpc_disposition_damage`. **UI wiring surfaced a real, live conflict**, correctly caught by discovery rather than shipped: `AssemblyWorkOrderDetail.tsx` already had a "Report Component Damage" dialog (`reportComponentIssue` in `production-api.ts`, marked `FIX 5B/5C`) writing to the exact same columns, with **incompatible semantics** — the legacy path overwrites `damage_qty` to a client-computed cumulative total per call, while the new RPC increments it. Running both would have silently corrupted live WIP damage tracking. Resolved by replacing the legacy damage-reporting mechanism entirely with the new RPCs. One thing deliberately left alone: the same legacy function also serves a genuinely different case — explaining an MIR *shortfall* (issued less than requested) — which isn't damage at all and doesn't fit the new RPCs' eligibility math; that path is untouched. Verified end-to-end against a real live item (V869) with full report→disposition→role-check→reconciliation proof, zero residual trace confirmed independently. **Not started:** weldment pool accumulation (`pool` batch type), sub-store (blocked on the client's qty-per mapping sheet). | Weldment, multi-batch, sub-store |
...
## 11a. Senior audit and fixes — 6 Sep 2026

A full audit of Phases 0–4 against the live schema, function bodies, grants, and frontend call sites (not just this doc) found two P0 issues, four P1 defects, and several P2 robustness gaps. All eight are fixed and deployed below. The two that were initially held for Vasu's sign-off (the cutover return-leg gap and the stock_in_process floor) were resolved the same day with approaches that avoid guessing at live data — see their own subsections.

### Fixed and deployed (DDL only, no data mutated)

| # | Problem | Fix |
|---|---|---|
| P0 | `authenticated`/`anon` held INSERT/UPDATE/DELETE/TRUNCATE on `job_card_stage_ledger` — RLS scopes by company but any logged-in user could forge ledger rows via PostgREST directly, bypassing every RPC guard. | Revoked write grants; `SELECT` only remains. |
| P1 | `_jcsl_reverse_qty_fifo` reversed by `(job_card_id, step_number, event)` with no reference to the specific DC line. Two DCs at the same stage — an explicit design goal (§4.6) — meant cancelling/reducing one could reverse the other's `issued` row instead. | Added optional `p_ref_type`/`p_ref_id` params; `rpc_cancel_dc` and `rpc_update_dc_line_qty` now pass the DC line's own id so FIFO scoping is per-line, not just per-stage. |
| P1 | `rpc_reverse_grn_return` had no guard on the `returned_rejected` path — reversing a rejection after the units were already sent to rework/scrap would drive undispositioned-rejected negative with no error. | Added the same net-non-negative guard used elsewhere, checked against `v_job_card_stage_ledger_totals`. Also added the missing legacy/status checks this function was missing versus its siblings. |
| P1 | `rpc_open_job_card` only checked that the *landing* stage was `entry_allowed`, not every stage being skipped below it. Since external and internal stages interleave at nearly every stage number in the live route data (confirmed: both types exist at stages 1–17), opening at a later internal stage could silently skip an external stage with no DC ever raised for it — exactly the gate bypass §8 claims doesn't exist. | Added a check that every stage in `[min_stage, entry_stage)` is itself `entry_allowed`, naming the first blocking stage in the error. **Deliberately not applied to `rpc_cutover_open_inflight_card`** — cutover reconstructs a batch's real history (those external stages genuinely happened via the pre-ledger DC/GRN flow); the guard would wrongly block legitimate reconstruction of the 27 deferred items. |
| P1 | `rpc_issue_dc` never checked that a DC line's item matched its linked job card's item; `rpc_confirm_grn_store` relied only on `prevent_dc_over_receipt` (which compares against `received_qty`, a column that can drift from `accepted_quantity + rejected_quantity` on a Stage-2 split edit) with no direct check against the ledger's own `issued` total. | Added an item-match guard to `rpc_issue_dc`; added a direct `Σ(already returned) + accepted + rejected ≤ Σ(issued)` check to `rpc_confirm_grn_store`, independent of the older trigger. Also added GRN/DC status checks (`deleted`/`cancelled`) to both, and the missing legacy/status checks to `rpc_undo_internal_step` and `rpc_dispose_rejected` (present on every other forward RPC, absent on these two). |
| P1 | `eligible_qty` in `v_job_card_stage_position` used `GREATEST(x, 0)` — a silent clamp of exactly the kind our own principles forbid, hiding the one signal (a negative) that would prove an accounting bug upstream. | Added `eligible_qty_raw` (unclamped) alongside the existing clamped `eligible_qty`. Verified 0 of 606 live rows are currently negative. Frontend/monitoring should watch `eligible_qty_raw`, not just `eligible_qty`. |

All of the above are dormant-safe: verified against live data before and after each change (0 live `returned_accepted`/`returned_rejected` rows exist yet; 0 dc_line_items are linked to non-legacy job cards yet — see next section), so none of this changed any live-visible behavior. They will matter the moment #1 below is resolved.

### Fixed and deployed — the stock_in_process floor (P1 #8)

Originally held back because the obvious fix (`RAISE` when a final-stage credit would drive `stock_in_process` negative) would have converted a data-quality problem into a production outage: the next GRN store-confirm or internal-step confirm on any of the 42 already-drifted items would throw and block whoever hit it. Reconciling the drift itself was explicitly out of scope — the sizes involved (V869 alone is short by 2,451 units) and the fact that `rpc_cutover_open_inflight_card` deliberately never touches stock buckets (correctly assuming the legacy DC-issue flow already moved the stock) point to this being pre-existing ledger/opening-stock drift unrelated to this session's work, not something safe to "correct" by picking a number here.

Deployed instead: `_jcsl_credit_if_final_stage` now floors `stock_in_process` at zero (it cannot physically go negative) but writes a loud `audit_log` row (`action = 'stock_in_process_shortfall'`) every time that floor is hit, capturing the item, job card, stage, credit quantity, and exact shortfall. `stock_free` still receives the full correct credit — availability downstream is never shortchanged — and no live transaction is blocked. Verified end-to-end via `BEGIN…ROLLBACK` against the worst real case (V869 / JW-26-27/419, full 2,451-unit shortfall): credit posted, `stock_free` correctly increased by the full amount, `stock_in_process` floored at 0 (not −2,451), one audit row written with the exact figures, zero exception raised, zero residual trace after rollback. The 42-item drift itself remains unreconciled and is now fully visible via `audit_log` the first time each one is hit, rather than invisible.

### Fixed and deployed — the cutover return-leg gap itself (P0 #1)

Originally flagged as needing Vasu's sign-off because a naive retroactive backfill by `(item, stage_number)` proved unsafe: 156 of the 161 cutover cards had exactly one matching open DC line by that key, but only 75 of those 156 had matching *quantities* — the other 81 overshot, often by a large margin (e.g. JW-26-27/419 / V869: card reconstructed at 2,451, matching open lines summed to 8,000). Root cause: **the same item can have multiple independent open DCs at the same stage** (the multi-batch case this whole redesign exists to support), so `(item, stage)` alone cannot uniquely reconstruct which specific DC line fed which specific card's net quantity — and the original cutover computation itself worked from actual GRN receipts, not from anything recoverable by re-summing today's open lines.

Rather than guess, built **confirm-at-receipt**: the decision is deferred to the one moment a human actually knows the truth — when the GRN for one of the 160 open cutover-era DCs is store-confirmed.

- **New column** `dc_line_items.job_card_link_reviewed` (boolean, default false) — marks a line as resolved (linked or explicitly not-applicable) so the same line is never re-prompted.
- **New function** `_jcsl_open_external_step(job_card_id)` — finds the single external stage on a card currently carrying outstanding (issued − returned) qty; returns NULL if that's ambiguous (0 or 2+ such stages), which the caller must treat as "ask a human."
- **New RPC** `rpc_get_job_card_link_candidates(dc_line_item_id)` — read-only, lists open non-legacy job cards for that line's item, for a picker UI.
- **New RPC** `rpc_get_pending_job_card_links(grn_id)` — read-only, call before store-confirm; returns only the lines that genuinely need a human decision (0 or 2+ candidate cards, or a single candidate with its own ambiguous position). Empty result means store-confirm can proceed with no prompt.
- **New RPC** `rpc_link_dc_line_to_job_card(dc_line_item_id, job_card_id, step_number DEFAULT NULL)` — the write. `job_card_id = NULL` records "explicitly not job-card-linked" without touching `job_card_id` itself.
- **`rpc_confirm_grn_store` extended**: for any unreviewed, unlinked line whose item has an active processing route, it now auto-resolves silently when exactly one open card exists for that item AND that card has exactly one open external stage — verified against live data at **243 of 424 open lines** auto-resolving cleanly with zero UI involvement. The remaining 50 lines (multiple candidate cards) and 60 lines (zero candidate cards, i.e. no open job card for that item at all) are correctly left for `rpc_get_pending_job_card_links` to surface, or fall through to the legacy credit path if nothing will ever match.
- **Real bug caught by the dry run itself, fixed before commit**: the first version of the over-return guard added to `rpc_confirm_grn_store` in this same session (P1 #6, below) compared against a *per-DC-line* issued total — which is always zero for a freshly-linked cutover line, because a cutover card's `issued` ledger row is posted once at `ref_type='job_card'` for the whole stage, not per line. That would have hard-blocked every legitimate cutover return the moment this shipped. Rewritten to check the shared *(job_card, step)* budget instead (`issued − returned` from `v_job_card_stage_ledger_totals`), which is both correct for cutover-linked lines and more correct for the ordinary multi-DC-per-stage case the original per-line check was too strict for.
- Verified end-to-end via `BEGIN…ROLLBACK` against real live data: a real unlinked line auto-resolved, correctly posted `returned_accepted` at an intermediate (non-final) stage with **no stock credit** (verified `stock_free` unchanged, since the linked stage wasn't the card's final stage — confirms `_jcsl_credit_if_final_stage` isn't over-firing); a real two-candidate line correctly surfaced both real, distinguishable cards via the picker RPC; the explicit "not linked" path correctly set `job_card_link_reviewed = true` without setting `job_card_id`. Zero residual trace confirmed by re-reading the same rows after rollback.
