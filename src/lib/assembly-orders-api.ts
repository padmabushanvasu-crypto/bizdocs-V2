...
/**
 * recordBuild — single-click build recording for pull-based assembly.
 * Creates a completed assembly order, serial numbers, FAT drafts, and does BOM backflush.
 */
export async function recordBuild(data: {
  ...
  fat_certificate_ids: string[];
}> {
  const companyId = await getCompanyId();

  // Step 1: Create assembly order as completed
  const { data: ao, error: aoError } = await (supabase as any)
    .from("assembly_orders")
    .insert({
      company_id: companyId,
      ao_number: "",
      ao_date: data.date_built,
      item_id: data.item_id,
      item_description: data.item_description,
      quantity_to_build: data.quantity,
      quantity_built: data.quantity,
      status: "completed",
      notes: data.notes ?? null,
      production_trigger: "manual",
      serial_numbers_generated: true,
      fat_drafts_created: true,
      backflushed: true,
      completed_at: new Date(data.date_built).toISOString(),
    })
    .select()
    .single();
  if (aoError) throw aoError;
  ...
  // Step 2: Generate serial numbers (in_stock)
  const snInserts = Array.from({ length: data.quantity }, (_, i) => ({
    company_id: companyId,
    serial_number: `${created.ao_number}/${String(i + 1).padStart(2, "0")}`,
    item_id: data.item_id,
    item_description: data.item_description,
  ...
  const { data: insertedSNs, error: snError } = await (supabase as any)
    .from("serial_numbers").insert(snInserts).select();
  if (snError) throw snError;
  const serialRows = (insertedSNs ?? []) as any[];
  const serialNumbersList: string[] = serialRows.map((s: any) => s.serial_number);

  // Step 3: Create FAT certificate drafts
  const fatIds: string[] = [];
  for (const sn of serialRows) {
    const fatNumber = await getNextDocNumber("fat_certificates", "fat_number", companyId, "fat_prefix");
    const { data: fatRow, error: fatError } = await (supabase as any)
      .from("fat_certificates")
      .insert({
        company_id: companyId,
        fat_number: fatNumber || "",
        fat_date: data.date_built,
        serial_number_id: sn.id,
        serial_number: sn.serial_number,
        item_id: data.item_id,
        item_description: data.item_description,
      ...
      .single();
    if (fatError) console.error("FAT draft create error:", fatError);
    else if (fatRow) fatIds.push((fatRow as any).id);
  }

  // Step 4: BOM backflush (if BOM exists)
  ...
  if (bomLines && bomLines.length > 0) {
    for (const bl of bomLines as any[]) {
      const requiredQty = (bl.quantity ?? 1) * data.quantity;
      const { data: compItem } = await (supabase as any)
        .from("items")
        .select("id, item_code, description, current_stock")
        .eq("id", bl.child_item_id)
        .single();
      if (!compItem) continue;
      const newStock = Math.max(0, ((compItem as any).current_stock ?? 0) - requiredQty);
      await (supabase as any).from("items").update({ current_stock: newStock }).eq("id", bl.child_item_id);
      await (supabase as any).from("stock_ledger").insert({
        company_id: companyId,
        item_id: bl.child_item_id,
        item_code: (compItem as any).item_code,
        item_description: (compItem as any).description,
        transaction_date: data.date_built,
        transaction_type: "assembly_consumption",
        qty_in: 0,
        qty_out: requiredQty,
        balance_qty: newStock,
        ...
        reference_number: created.ao_number,
        notes: `BOM backflush: ${created.ao_number}`,
        created_by: null,
  ...
  // Step 5: Update finished goods stock for the assembled item
  const { data: parentItem } = await (supabase as any)
    .from("items")
    .select("stock_finished_goods, current_stock")
    .eq("id", data.item_id)
    .single();
  const newFG = ((parentItem as any)?.stock_finished_goods ?? 0) + data.quantity;
  const newTotal = ((parentItem as any)?.current_stock ?? 0) + data.quantity;
  await (supabase as any).from("items").update({ stock_finished_goods: newFG, current_stock: newTotal }).eq("id", data.item_id);
  ...
  await (supabase as any).from("stock_ledger").insert({
    ...
    qty_out: 0,
    balance_qty: newTotal,
    unit_cost: 0,
    total_value: 0,
    reference_type: "assembly_order",
    reference_id: created.id,
    reference_number: created.ao_number,
    notes: `Record Build: ${created.ao_number}`,
    created_by: null,
    from_state: null,
    to_state: "finished_goods",
  });

  logAudit("assembly_order", created.id, "Build Recorded", {
    summary: `${created.ao_number} — ${data.item_description} × ${data.quantity} built on ${data.date_built}`,
    ao_number: created.ao_number,
    quantity_built: data.quantity,
    serials_generated: serialNumbersList.length,
  }).catch(console.error);

  return {
    assembly_order_id: created.id,
    serial_numbers: serialNumbersList,
    fat_certificate_ids: fatIds,
  };
}
