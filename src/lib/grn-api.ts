...
export async function createGRN({ grn, lineItems }: CreateGRNData) {
  ...
  const { data: newGRN, error } = await supabase.from("grns").insert({
    ...
    po_id: grn.po_id, po_number: grn.po_number,
    vendor_id: grn.vendor_id, vendor_name: grn.vendor_name,
    vendor_invoice_number: grn.vendor_invoice_number, vendor_invoice_date: grn.vendor_invoice_date,
    vehicle_number: grn.vehicle_number, lr_reference: grn.lr_reference,
