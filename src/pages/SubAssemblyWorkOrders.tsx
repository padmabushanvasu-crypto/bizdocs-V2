...
export default function SubAssemblyWorkOrders() {
  ...
  const { data: awos = [], isLoading } = useQuery({
    queryKey: ["awo", "sub_assembly", month, showDeleted],
    queryFn: () => fetchAssemblyWorkOrders({ type: "sub_assembly", month: month || undefined, showDeleted }),
  });

  const { data: stats } = useQuery({
    queryKey: ["awo-stats", "sub_assembly"],
    queryFn: () => fetchAwoStats("sub_assembly"),
  });
    ...
    <div className="p-4 md:p-6 space-y-4">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold">Sub-Assembly Work Orders</h1>
        </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) setForm(defaultForm); }}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Raise New Sub-Assembly Work Order</DialogTitle>
          </DialogHeader>
      ...
      <AwoDeleteDialog
        ...
        onDeleted={() => {
          ...
          queryClient.invalidateQueries({ queryKey: ["awo", "sub_assembly"] });
          queryClient.invalidateQueries({ queryKey: ["awo-stats", "sub_assembly"] });
        }}
      />
    </div>
  );
