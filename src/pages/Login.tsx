    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-primary flex-col items-center justify-center p-12 text-primary-foreground">
        <div className="max-w-md text-center space-y-8">
          <h1 className="font-display text-5xl font-extrabold tracking-tight">BizDocs</h1>
          <p className="text-lg opacity-90">Replace your Excel sheets with smart, GST-ready documents</p>
          <div className="space-y-4 text-left mt-8">
            {[
              "Purchase Orders, Delivery Challans & Invoices",
              "Automatic GST calculation — CGST, SGST & IGST",
              "Clean PDFs ready to print or share on WhatsApp",
            ].map((text) => (
              <div key={text} className="flex items-start gap-3">
                <Check className="h-5 w-5 mt-0.5 shrink-0 text-accent" />
              ...
              <form onSubmit={handleForgotPassword} className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="forgot-email" type="email" required value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} className="pl-10" placeholder="you@company.com" />
                  </div>
                </div>
                <Button type="submit" className="w-full" disabled={loading}>{loading ? "Sending…" : "Send Reset Link"}</Button>
                <button type="button" onClick={() => setForgotMode(false)} className="text-sm text-primary hover:underline w-full text-center">← Back to Sign In</button>
              </form>
            ) : (
              <>
